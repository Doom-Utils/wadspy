/*
  WADSpy v1.1 (2012-07-01) Estimate a Doom level's difficulty
  Copyright (c) 2000-2012  Oliver Brakmann <obrakmann@gmx.net>
...
void countlinedefs(FILE *f, DWord offs, DWord len)
{
    ...
    printf("\tTriggers / linedefs    %4d / %4d\n", ntrig, nlines);
...
void countsectors(FILE *f, DWord offs, DWord len)
{
    ...
    printf("\tAverage brightness            %4d (0=dark, 255=bright)\n", bright / nsec);

    printf("\tSecrets                       %4d\n", nsecret);
    printf("\tNukeage / sectors      %4d / %4d\n", nnuke, nsec);
